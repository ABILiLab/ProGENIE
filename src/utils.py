        ...
        for wsi in wsis_with_features:
            try:
                with h5py.File(os.path.join(feature_path, proj, wsi, wsi+'.h5'), "r") as f:
                    cols = list(f.keys())
                    if feature_name not in cols:
                        print(f"❌ {wsi} does not contain feature '{feature_name}'")
                        remove.append(wsi)
            except Exception as e:
                print(f"⚠️ Error reading {wsi}: {e}")
                remove.append(wsi)
        all_wsis_with_features += wsis_with_features
    to_remove = df[~df['wsi_file_name'].isin(all_wsis_with_features)].wsi_file_name.values.tolist()
    for wsi in to_remove:
        print(f"❌ {wsi} not found in feature directory")
    remove += to_remove
    print(f'Original shape: {df.shape}')
    df = df[~df['wsi_file_name'].isin(remove)].reset_index(drop=True)
    print(f'New shape: {df.shape}')
